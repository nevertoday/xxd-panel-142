...
- Text: `prompt` or `exact`; `none` requires clarification; resolve the target locale explicitly.
...
Core typography is the subject structure, not optional decoration. For `none`, clarify whether only supporting copy should be omitted or the user accepts another non-typographic style. Do not claim a fully text-free result preserves this style. Exact wording supplies the core text verbatim.
