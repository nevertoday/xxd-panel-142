...
Text modes are `prompt` and `exact`; a request for `none` requires clarification because letterforms are the defining image structure. Ask whether the user means no supporting copy (retain core typography) or accepts a different non-typographic style. Never silently replace this style or promise full text removal while retaining it. Resolve the target locale explicitly. Exact text supplies the core wording verbatim; do not invent replacement glyph text. Only supporting copy can be omitted without losing this style. In prompt text mode, derive concise source-grounded core wording and only minimal supporting copy in the resolved locale, following the source brief's exact typography, scale, material and placement instructions.
...
- `references/xxd-panel-142-prompt.zh-CN.md` and `.en.md` — delivery adapter notes
